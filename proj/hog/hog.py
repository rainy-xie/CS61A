...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    def strategy_always_roll(score,opponent_score):
        return n
    return strategy_always_roll
    # END PROBLEM 6
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while score<=goal:
        ...
        while opponent_score<=goal:
            if roll != strategy(score,opponent_score):
                return False
            opponent_score +=1
        score+=1
    return True         
...
def make_averaged(original_function, total_samples=1000):
    ...
    def caculate_average(*args):
        ...
        total = total_samples
        while total:       
            mysum += original_function(*args)
            total = total-1
        average = mysum/total_samples
        return average
    return caculate_average
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    "*** YOUR CODE HERE ***"
    max_scoring_roll = 1
    max_scoring_num = make_averaged(roll_dice,total_samples)(max_scoring_roll,dice)
    
    roll_num = 1
    while roll_num < 10:
        roll_num += 1
        average = make_averaged(roll_dice,total_samples)(roll_num,dice)
        if average > max_scoring_num:
            max_scoring_num = average
            max_scoring_roll = roll_num
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    tail_score = tail_points(opponent_score)
    test_score = tail_score + score
    if perfect_square(test_score):
        test_score = next_perfect_square(test_score)
    
    if (test_score-score) >= threshold:
        return 0
    else:
        return num_rolls
