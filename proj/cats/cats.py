...
def pick(paragraphs, select, k):
    ...
    selected = []
    i = 0
    # print("DEBUG:", paragraphs[i])
    while i < len(paragraphs):
        # print("DEBUG:", select(paragraphs[i]))
        if select(paragraphs[i]):
            selected = selected+[paragraphs[i]]
        i += 1
    if k < len(selected):
        return selected[k]
    else:
...
def about(subject):
    ...
    def select(paragraph):
        ...
        paragraph_to_word = split(paragraph)
        for i in range(len(paragraph_to_word)):
            for j in range(len(subject)):
                if lower(paragraph_to_word[i]) == subject[j]:
                    return True
        return False
...
def accuracy(typed, source):
    ...
    for i in range(length):
        if typed_words[i] == source_words[i]:
            accurate_words += 1
    return (accurate_words / len(typed_words))*100.0
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    diff = []
    for i in range(len(word_list)):
        diff = diff + [diff_function(typed_word, word_list[i], limit)]
    if min(diff, key=abs) > limit:
        return typed_word
    else:
        for i in range(len(word_list)):
            if diff_function(typed_word, word_list[i], limit) == min(diff, key=abs):
                return word_list[i]
    # returned_word = min(word_list,key = diff_function(typed_word,word_list,limit))
    # print("DEBUG:", returned_word)
...
def feline_fixes(typed, source, limit):
    ...
    #     if typed
    #     return
    # return helper(typed,source,limit) + abs(len(typed)-len(source))
    def helper(n, count):
        if count > limit:
            return limit + 1    # 超过limit时返回limit+1表示递归结束
        if n >= min(len(typed), len(source)):
            return count
        if typed[n] != source[n]:
            return helper(n+1, count+1)
        else:
            return helper(n+1, count)
    return helper(0, 0) + abs(len(typed)-len(source))
...
def time_per_word(words, times_per_player):
    ...
    for i in range(len(times_per_player)):
        ...
        for j in range(len(times_per_player[i])-1):
            period = period + [times_per_player[i][j+1]-times_per_player[i][j]]
        times = times + [period]
...
def fastest_words(match):
    ...
    for word in range(len(get_all_words(match))):
        min_time = time(match,0,word)
        this_player = 0
        for player in range(1,len(get_all_times(match))):
            if min_time > time(match,player,word):
                min_time = time(match,player,word)
                this_player = player
        return_list[this_player] += [get_word(match,word)]
